...
clo %>% select(Host, Virus) %>%
  distinct() %>% 
  filter(!Host=='homo sapiens') -> clo1

clo <- read_csv("~/Documents/Github/twothings/Data/helminths.csv")
clo %>% rename(Virus = Parasite) %>%
  filter(!Host=='homo sapiens') %>% distinct() -> clo2
